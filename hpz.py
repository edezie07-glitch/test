...
# ========== CONFIGURATION ==========
class Config:
    SECRET_KEY = os.environ.get('SECRET_KEY', 'hpz-secret-key-2024-very-long-and-secure')
...
def chat_page():
    try:
        ...
        print(f"✅ Loading chat for {user.username} (ID: {user.id})")
...
def register():
    try:
        ...
        print(f"   Session: user_id={session.get('user_id')}, username={session.get('username')}")
...
if __name__ == '__main__':
    ...
    print(f"   Port: {port}")
    print(f"   Features: ✅ Friends ✅ Search ✅ Messages ✅ Files")
    print(f"{'='*50}\n")
