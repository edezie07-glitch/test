...
def format_message(msg, user_id):
    ...
    reply_msg = None
    if msg.reply_to_id:
        rep = Message.query.get(msg.reply_to_id)
...
def handle_connect():
    ...
    # Auto-join all friend chat rooms
    fs = Friendship.query.filter(or_(Friendship.user1_id==uid, Friendship.user2_id==uid)).all()
    for f in fs:
        fid = f.user2_id if f.user1_id==uid else f.user1_id
        join_room(get_chat_id(uid, fid))
        if fid in online_users:
            emit('user_online', {'user_id':uid,'online':True}, room=online_users[fid]['sid'])
...
def handle_disconnect():
    ...
    for f in fs:
        fid = f.user2_id if f.user1_id==uid else f.user1_id
        if fid in online_users:
            emit('user_online', {'user_id':uid,'online':False}, room=online_users[fid]['sid'])
